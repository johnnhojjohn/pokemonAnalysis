...
Making a pivot table with stats all in one column
```{r}
...
```
All stats with the exception of total is all grouped similarily but.. total is a bit odd.
...
Let's pivot wider the table back to normal

```{r}
pokemon <- poke_pivot_long %>%
  pivot_wider(names_from = stats, values_from = y)
...
detect outliers by type, and other columns
